...
Call log:
  - waiting for locator('button:has-text("Add to Shopping Cart")')

    at CartPage.addToCart (/home/webosmotic/Documents/BARU/Baru_automation/Baru_automation/Pages/cart.page.js:19:32)
    at /home/webosmotic/Documents/BARU/Baru_automation/Baru_automation/tests/Baru.spec.js:34:3
```
...
```ts
   ...
   3 |   constructor(page) {
   4 |     this.page = page;
   5 |     this.addToCartButton = page.locator('button:has-text("Add to Shopping Cart")'); // Match exact case
   6 |     this.viewCartButton = page.locator('a:has-text("View Cart")');
   7 |   }
   8 |
   9 |   async addToCart() {
  10 |     await this.page.waitForTimeout(2000); // Wait for DOM updates
  11 |
  12 |     const isVisible = await this.addToCartButton.isVisible();
  13 |
  14 |     if (!isVisible) {
  15 |       console.log('❗ ADD TO CART button is not visible');
  16 |       await this.page.pause(); // Let tester inspect
  17 |     }
  18 |
> 19 |     await this.addToCartButton.scrollIntoViewIfNeeded();
     |                                ^ TimeoutError: locator.scrollIntoViewIfNeeded: Timeout 10000ms exceeded.
  20 |     await this.addToCartButton.click({ force: true });
  21 |   }
  22 | }
  23 |
