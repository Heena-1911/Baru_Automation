...
Call log:
  - waiting for locator('button:has-text("Proceed to Checkout")') to be visible
...
```ts
  ...
  13 |     this.proceedToCheckoutButton = page.locator('button:has-text("Proceed to Checkout")'); // Added locator
  14 |   }
  15 |
  16 |   // Proceed to checkout
  17 |   async proceedToCheckout() {
> 18 |     await this.proceedToCheckoutButton.waitFor({ state: 'visible', timeout: 10000 });
     |                                        ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
