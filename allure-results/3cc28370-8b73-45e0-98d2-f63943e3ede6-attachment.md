...
Locator: locator('button:has-text("VIEW IN CART")')
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 10000ms
  - waiting for locator('button:has-text("VIEW IN CART")')

    at /home/webosmotic/Documents/BARU/Baru_automation/Baru_automation/tests/Baru.spec.js:37:40
```
...
```ts
  ...
  34 |   console.log('✅ Item added to cart');
  35 |
  36 |   // Wait for "VIEW IN CART" button to appear after item is added to cart
> 37 |   await expect(cartPage.viewInCartBtn).toBeVisible({ timeout: 10000 });
     |                                        ^ Error: Timed out 10000ms waiting for expect(locator).toBeVisible()
  38 |
  39 |   // View Cart
  40 |   await cartPage.viewInCartBtn.click();
  41 |   console.log('✅ Viewing cart');
  42 |
  43 |   // Proceed to checkout
  44 |   await checkoutPage.proceedToCheckout();
  45 |   console.log('✅ Proceeded to checkout');
  46 |
  47 |   // Fill checkout details
  48 |   await checkoutPage.fillCheckoutDetails('Heena', 'Webosmotic', 'heena.webosmotic@gmail.com');
  49 |   console.log('✅ Checkout details filled');
  50 |
  51 |   // Place Order
  52 |   await orderPage.placeOrder();
  53 |   console.log('✅ Clicked on "PLACE ORDER"');
  54 |
  55 |   // Confirm Order
  56 |   await orderPage.confirmOrder();
  57 |   console.log('✅ Order confirmed');
  58 | });
  59 |
```