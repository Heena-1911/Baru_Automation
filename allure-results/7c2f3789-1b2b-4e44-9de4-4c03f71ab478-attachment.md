...
Call log:
  - waiting for locator('input[name="password"]')
    ...
    at LoginPage.login (/home/webosmotic/Documents/BARU/Baru_automation/Baru_automation/Pages/login.page.js:12:32)
    at /home/webosmotic/Documents/BARU/Baru_automation/Baru_automation/tests/Baru.spec.js:18:3
```
...
- textbox "Name"
- textbox "Email": heena.webosmotic@gmail.com
...
```ts
   ...
   3 |     constructor(page) {
   4 |       this.page = page;
   5 |       this.emailInput = page.locator('input[name="email"]');
   6 |       this.passwordInput = page.locator('input[name="password"]');
   7 |       this.loginButton = page.locator('button[type="submit"]');
   8 |     }
   9 |   
  10 |     async login(email, password) {
  11 |       await this.emailInput.fill(email);
> 12 |       await this.passwordInput.fill(password);
     |                                ^ TimeoutError: locator.fill: Timeout 10000ms exceeded.
